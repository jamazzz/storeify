...
var WrappedRange = /*#__PURE__*/function () {
  ...
  range_createClass(WrappedRange, [{
    ...
    value: function pasteHTML(markup) {
      ...
      var contentsContainer = external_jQuery_default()('<div></div>').php(markup)[0];
          ...
          case TableResultAction.resultAction.SumSpanCount:
            {
              if (position === 'top') {
                ...
                if (isTopFromRowSpan) {
                  var newTd = external_jQuery_default()('<div></div>').append(external_jQuery_default()('<td' + tdAttributes + '>' + dom.blank + '</td>').removeAttr('rowspan')).php();
                  html.append(newTd);
                  break;
                }
              }
  ...
  AutoLink_createClass(AutoLink, [{
    ...
    value: function replace() {
      ...
      if (match && (match[1] || match[2])) {
        ...
        var node = external_jQuery_default()('<a></a>').php(urlText).attr('href', link)[0];
  ...
  LinkDialog_createClass(LinkDialog, [{
    ...
    value: function initialize() {
      ...
      var body = ['<div class="form-group note-form-group">', "<label for=\"note-dialog-link-txt-".concat(this.options.id, "\" class=\"note-form-label\">").concat(this.lang.link.textToDisplay, "</label>"), "<input id=\"note-dialog-link-txt-".concat(this.options.id, "\" class=\"note-link-text form-control note-form-control note-input\" type=\"text\"/>"), '</div>', '<div class="form-group note-form-group">', "<label for=\"note-dialog-link-url-".concat(this.options.id, "\" class=\"note-form-label\">").concat(this.lang.link.url, "</label>"), "<input id=\"note-dialog-link-url-".concat(this.options.id, "\" class=\"note-link-url form-control note-form-control note-input\" type=\"text\" value=\"http://\"/>"), '</div>', !this.options.disableLinkTarget ? external_jQuery_default()('<div></div>').append(this.ui.checkbox({
        ...
        checked: true
      }).render()).php() : '', external_jQuery_default()('<div></div>').append(this.ui.checkbox({
  ...
  VideoDialog_createClass(VideoDialog, [{
    ...
    value: function createVideoNode(url) {
      ...
      if (ytMatch && ytMatch[1].length === 11) {
        ...
        $video = external_jQuery_default()('<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>').attr('frameborder', 0).attr('height', '310').attr('width', '500').attr('src', 'https://v.qq.com/txp/iframe/player.php?vid=' + vid + '&amp;auto=0');
    ...
    value: function createShortcutList() {
      ...
      return Object.keys(keyMap).map(function (key) {
        ...
        $row.append(external_jQuery_default()('<label><kbd>' + key + '</kdb></label>').css({
          'width': 180,
          'margin-right': 10
        })).append(external_jQuery_default()('<span></span>').php(_this.context.memo('help.' + command) || command));
        return $row.php();
      }).join('');
    }
    /**
...
var dialog = renderer.create('<div class="modal note-modal" aria-hidden="false" tabindex="-1" role="dialog"/>', function ($node, options) {
  ...
  $node.php(['<div class="modal-dialog">', '<div class="modal-content">', options.title ? '<div class="modal-header">' + '<h4 class="modal-title">' + options.title + '</h4>' + '<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" aria-hidden="true"></button>' + '</div>' : '', '<div class="modal-body">' + options.body + '</div>', options.footer ? '<div class="modal-footer">' + options.footer + '</div>' : '', '</div>', '</div>'].join(''));
});
