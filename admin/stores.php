            ...
            <div class="card-body">
              ...
              <table class="table table-responsive-sm table-striped table-responsive-md">
                ...
                <tbody>
                  <?php
                  $filter = $_POST['search'] ?? '';
                  $select2 = "SELECT * FROM websites WHERE subdomain LIKE '%$filter%' OR id LIKE '%$filter%'OR name LIKE '%$filter%'";
                  ...
                  while ($row2 = mysqli_fetch_assoc($result2)) {
                    ...
                    <tr>
                      <td>' . $row2['id'] . '</td>
                      <td>' . $row2['name'] . '</td>
                      <td>' . $row2['subdomain'] . '</td>
                      <td><img src="'.$row2['logo'].'" alt="Image"></td>
                      <td class="text-end">
                          <form action="/storeify/admin/delete.php" method="POST">
                            <input type="hidden" name="websiteID" value="' . $row2['id'] . '">
